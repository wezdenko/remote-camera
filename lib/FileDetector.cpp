...
std::string FileDetector::getPath(){
    return path;
}

std::vector<std::string> FileDetector::getFilesName(std::function<bool(std::string&)> selectFunc){
...
void FileDetector::setPath(std::string path){
    this->path = path;
}